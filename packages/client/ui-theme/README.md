...
`base.css` also defines the frameless desktop title-bar inset: 44px for macOS, Windows, and Linux, with a 40px fallback for another desktop marker. For documents marked `data-dsh-desktop-platform="darwin"` or `"win32"`, it makes the `html`/`body`/`#root` stack transparent so the Electron window material can reach layout-owned translucent regions. Web documents and Linux desktop documents retain the base background.
