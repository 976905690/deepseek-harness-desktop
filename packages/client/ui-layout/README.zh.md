...
Electron 外壳将 `html[data-dsh-desktop-platform]` 标为 `darwin` 或 `win32` 时，AppFrame 只让自身框架与侧栏列半透明，使原生窗口材质透过侧栏；会话列和详情列继续绘制 `--dsw-alias-bg-base`。Web 页面与 Linux 桌面窗口不匹配这些材质选择器，因此仍使用普通的不透明侧栏。Windows 与 Linux 的工作列会预留原生标题栏覆盖层高度，macOS 红绿灯则只占用侧栏预留区域。
