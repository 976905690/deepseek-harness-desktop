...
The BrowserWindow is frameless on every platform. macOS uses an inset hidden title bar, sidebar vibrancy, and an explicit traffic-light position; its collapsed sidebar resolves to an 88px rail whose 36px controls are centered, and the top control aligns with the expanded logo row below the traffic lights. Windows uses acrylic material. The window itself allows transparency, but Linux keeps the Web client's opaque surfaces because Electron provides no equivalent native material there. The window remains hidden until the renderer is loaded with its desktop presentation marker.

Electron appends one allowlisted platform value to the validated Host URL. The Web entry consumes it before mounting the client tree, marks the root element as a desktop document, and records the platform. This static marker grants no preload or IPC capability. Client styles use it only for native presentation and title-bar hit regions. The resident conversation root always mounts a center-column drag band, including when no Session header is visible; a visible conversation header paints above that band and becomes draggable itself. Buttons, links, form fields, ARIA controls, tabs, and editable descendants inside the header are explicit no-drag regions so window movement does not consume their interaction. The sidebar owns its separate drag strip and applies the same exclusion to its controls.
