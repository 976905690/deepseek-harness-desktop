...
export interface AdvancedFrameInjected {
  ...
  /** Product title drawn in the native caption row (Threerouter extension). */
  windowTitle?: string
}
...
export function DesktopOwnedFrame({ layout, mode, platform, windowTitle, renderSlot, useSessions }: AdvancedFrameProps & {
    ...
    >
      {mode === 'advanced' && platform === 'darwin' && (
        <div className="dshDesktopMacCaptionRow" aria-hidden="true">
          {windowTitle !== undefined && <span className="dshDesktopCaptionTitle">{windowTitle}</span>}
        </div>
      )}
