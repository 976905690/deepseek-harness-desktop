/**
 * 桌面自有媒体 toolview：generate_video / generate_image 的 keyed
 ...
 * 客户端（无桌面覆盖）。上游类型（ClientContext）type-only import，构建后
 * 擦除，不进 client bundle；toolview 的 props 用本地结构视图，避免引入
 * dsh-client-ui-tool 类型入口的 augment 缺陷（见 MediaToolviewProps 注释）。
 *
 * @module dsh-plugin-desktop/client/media-toolview
 */

import { useState } from 'react'
import type { ClientContext } from '@deepseek-ai/dsh-client-runtime/client'
// 仅拉类型面（'tool.call.toolview' slot augment），不导入具名类型；配合
// tsconfig.client.json 的 skipLibCheck：上游该入口的声明把上游构建时才
 ...
 * 自动级联注销（与 desktop-settings.ts 的注入方式一致），无需外层 ctx.effect。
